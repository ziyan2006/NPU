...
| P3 架构/ISA 探索 | 数据流、存储层次、ISA、周期模型 | 性能和 BRAM 预算闭合 | 未冻结 |
| P4 微架构规格 | 模块接口、流水线、时序、异常行为 | RTL 接口和逐周期行为可实现 | 未开始 |
| P5 可执行参考 | 图编译器、位精确模拟器、测试向量 | 所有 P0 指令有 golden | 未开始 |
| P6 RTL/HLS 实现 | 可综合模块和软件驱动 | 模块仿真通过 | 未开始 |
| P7 集成验证 | SoC、DMA、中断、CDC、回归 | 覆盖率和需求回归闭合 | 未开始 |
| P8 实现收敛 | 综合、布局布线、时序、功耗 | 资源和时钟满足规格 | 未开始 |
| P9 上板验收 | 长稳、实时性、音频效果 | 所有 P0 板级测试通过 | 未开始 |

当前不能直接进入 P6。最近的正式里程碑是完成 P1，并用编译器原型、存储规划器和周期模型关闭 P2/P3 风险。
