...
当前网络 A 的软件证据：248 个 tile 展开为 1,869 条命令，64-lane 的 transaction 调度为 2,485,596 cycle，DDR 流量 3,512,000 B；全部 1,078 条 DMA command 已解析为 45,664 个 AXI burst，所有 allocation/bank/对齐/4 KiB 边界通过，静态访问区间冲突为 0。O8I8 权重仅增加约 0.23%。这支持 ADR-010/012/016/018/020 继续进入 RTL 原型，但在第二网络、实际 BRAM 端口、AXI 实效和 post-route 结果出现前仍不升级为“已接受”。
