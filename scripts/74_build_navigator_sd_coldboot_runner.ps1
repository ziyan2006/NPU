...
$ps7 = 'hardware/build/navigator_z7020_vendor_v37_export'
$out = 'hardware/build/navigator_sd_coldboot_runner'
foreach ($file in @($gcc, $objcopy, "$payload/navigator_npu_task_payload.c", "$payload/navigator_npu_task_payload.h", "$ps7/ps7_init.c", "$ps7/ps7_init.h")) {
...
& $gcc -mcpu=cortex-a9 -marm -mfloat-abi=soft -O2 -ffreestanding -fno-builtin -fdata-sections -ffunction-sections -nostdlib `
  '-Isoftware/include' '-Isoftware/bringup/minimal_a9/ps7_compat' "-I$payload" "-I$ps7" '-Wl,-T,software/bringup/minimal_a9/linker_sd_coldboot.ld' '-Wl,--gc-sections' `
  ...
  software/src/npu_driver.c "$payload/navigator_npu_task_payload.c" "$ps7/ps7_init.c" `
